...
def _scorers_prompt(today: str) -> str:
    return (
        ...
        f"Сегодня {today}. Нужен список лучших бомбардиров — до 5 игроков, "
        "отсортированных по убыванию голов. Включай и игроков с 1 голом, чтобы "
        "по возможности набрать 5 человек; при равенстве голов добавляй всех. "
        ...
        "Если турнир только стартовал и данных совсем нет — верни пустой items."
    )
...
def _assists_prompt(today: str) -> str:
    return (
        "Найди в интернете актуальную статистику Чемпионата мира по футболу 2026 "
        "(FIFA World Cup 2026). Источник — официальный FIFA. "
        f"Сегодня {today}. Нужен список лидеров по голевым передачам (ассистам) — "
        "до 5 игроков, отсортированных по убыванию ассистов. Включай и игроков с "
        "1 ассистом, чтобы по возможности набрать 5 человек; при равенстве "
        "добавляй всех. "
        "Верни ТОЛЬКО валидный JSON без markdown и без каких-либо пояснений, "
        "строго по схеме: "
